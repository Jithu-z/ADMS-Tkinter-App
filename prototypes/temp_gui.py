...
class admsapp():
    ...
    def book_ticket6(self,flight,seats,seatno):
        ...
        tk.Button(self.root,text="Confirm and finish",command=lambda: self.book_ticket9(flight,seats)).pack()
        tk.Button(self.root,text="Add Luggage",command=lambda: self.book_ticket7(flight,seats)).pack()
        tk.Button(self.root,text="Cancel",command=lambda: self.book_ticket4(flight)).pack()
    def book_ticket7(self,flight,seats):
        for widget in self.root.winfo_children():
            widget.destroy()
        tk.Label(self.root,text="Select Luggage Option").pack()
        luggage=adms.get_luggage(0)
        self.luggage_var = tk.IntVar(value=0)
        tk.Radiobutton(self.root, text="No Luggage\t"+"(Rs0)", variable=self.luggage_var, value=0).pack()
        for i in range(len(luggage)):
            tk.Radiobutton(self.root, text=luggage[i][1]+"\t"+"(Rs"+str(luggage[i][2])+")", variable=self.luggage_var, value=luggage[i][0]).pack()
        tk.Button(self.root,text="Confirm",command=lambda: self.book_ticket8(flight,seats,self.luggage_var.get())).pack()
        tk.Button(self.root,text="Cancel",command=lambda: self.book_ticket6(flight,seats,len(self.seat_id.split()))).pack()
    def book_ticket8(self,flight,seats,luggage):
        ...
        for widget in self.root.winfo_children():
            widget.destroy()
        tk.Label(self.root,text="Total Cost: Rs"+str(total_cost)).pack()
        tk.Label(self.root,text=str(len(self.seat_id.split()))+" "+seats[1]+" class seats (Rs"+str(seats[2])+"/ticket)").pack()
        if(luggage==0):
            tk.Label(self.root,text="No Luggage").pack()
        else:
            tk.Label(self.root,text=lug[1]+" (Rs"+str(lug[2])+")").pack()
        tk.Button(self.root,text="Confirm and finish",command=lambda: self.book_ticket9(flight,seats,lug[1],lug[2],total_cost)).pack()
        tk.Button(self.root,text="Cancel",command=lambda: self.book_ticket6(flight,seats,len(self.seat_id.split()))).pack()
    ...
    def view_bill(self):
        ...
        tk.Label(self.root,text="Cost per seat:"+str(adms.getseat_info(ticket[0][5].strip(" class"))[2])).pack()
        tk.Label(self.root,text="Extra Luggage applied: "+ticket[0][7]).pack()
        tk.Label(self.root,text="Luggage Cost: Rs"+str(ticket[0][8])).pack()
