...
class admsapp():
    ...
    def _clear_widgets(self):
        """Helper function to destroy all widgets in the root window."""
    ...
    def create_login_widgets(self):
        ...
        ttk.Button(button_frame,text="Login",command=self.login).pack(pady=10)
        ttk.Button(button_frame,text="Register",command=self.register).pack(pady=10)
    ...
    def register2(self,mobno):
        if(adms.passignin(mobno) or len(mobno)!=10):
            ...
            ttk.Label(frame,text="Enter reg.date(YYYY-MM-DD):").grid(row=2, column=0, sticky="w", pady=5, padx=5)
            regdate=tk.StringVar()
            ttk.Entry(frame,textvariable=regdate, width=30).grid(row=2, column=1, pady=5, padx=5)

            ttk.Button(frame,text="Register",command=lambda: self.register_user(name.get(),mobno,regdate.get(),address.get())).grid(row=3, column=0, columnspan=2, pady=20)
    ...
    def user_buttons(self,i):
        if i == 0:
            ...
            if adms.hasbooked(self.mobno.get()):
                ...
                messagebox.showinfo("Flight Status", f"Your flight {status[0][0]} from {status[0][1]} to {status[0][2]} on {str(status[0][3])} is currently {status[0][4]}")
            else:
    ...
    def book_ticket2(self,source):
        ...
        for i in range(len(buttons)):
            ttk.Button(frame,text=buttons[i],command=lambda b=buttons[i]: self.book_ticket3(source,b)).pack(pady=5)
        ttk.Button(self.root,text="<--- Back",command=self.book_ticket).pack(pady=20)
    ...
    def book_ticket3(self,source,destination):
        ...
        ttk.Label(frame,text="Flight ID").grid(row=0, column=0, padx=10, pady=5)
        ttk.Label(frame,text="Date").grid(row=0, column=1, padx=10, pady=5)
        ttk.Label(frame,text="Status").grid(row=0, column=2, padx=10, pady=5)
        ...
        for i in range(len(flights)):
            ...
            # Fix: Use default argument in lambda to capture current flight
    ...
    def book_ticket4(self,flight):
        ...
        ttk.Button(btn_frame,text="<--- Back",command=lambda: self.book_ticket3(self.source,self.destination)).pack(side='left', padx=10)
    ...
    def book_ticket7(self,flight,seats):
        ...
        self.luggage_var = tk.IntVar(value=1)
    ...
    def book_ticket9(self,flight,seats,extra_lug="Nill",lug_rate=0,total_cost=0):
        ...
        if(adms.booktickets(adms.getid(self.mobno.get()),adms.getname(self.mobno.get()),flight[0],self.source,self.destination[0],seats[1],flight[1],extra_lug,lug_rate,self.rate,total_cost,self.seat_id)):
    ...
    def cancel_ticket(self):
        if adms.hasbooked(self.mobno.get()):
            ...
            ttk.Label(frame,text="Confirm Cancellation of Flight?", style='Header.TLabel').pack(pady=10)
            ttk.Label(frame,text=f"Flight {ticket[0][2]} from {ticket[0][3]} to {ticket[0][4]} on {ticket[0][6]}").pack(pady=10)
            
            btn_frame = ttk.Frame(self.root)
            btn_frame.pack(pady=20)
            ttk.Button(btn_frame,text="Confirm",command=self.cancel_ticket2).pack(side='left', padx=10)
            ttk.Button(btn_frame,text="Go Back",command=self.create_user_widgets).pack(side='left', padx=10)
        else:
    ...
    def view_bill(self):
        ...
        ticket=adms.getticket(adms.getid(self.mobno.get()))
        if not ticket:
            messagebox.showerror("Error", "Could not retrieve ticket information.")
            self.create_user_widgets()
            return

        seat_class_str = ticket[0][5].replace(' class', '').lower()
        seat_info = adms.getseat_info(seat_class_str)
        cost_per_seat = seat_info[2] if seat_info else "N/A"

        frame = ttk.Frame(self.root, padding="20", relief="groove")
        frame.pack(expand=True, pady=20, padx=20)
        
        ttk.Label(frame, text="Bill Details", style='Header.TLabel').grid(row=0, column=0, columnspan=2, pady=(0,15))

        details = {
            "Flight ID:": ticket[0][2], "From:": ticket[0][3], "To:": ticket[0][4], "Date:": str(ticket[0][6]), "Seat Type:": ticket[0][5],
            "Seats Booked:": str(len(ticket[0][11].split())),
            "Cost per seat:": f"Rs {cost_per_seat}",
            "Extra Luggage:": ticket[0][7],
            "Luggage Cost:": f"Rs {ticket[0][8]}",
            "Total Cost:": f"Rs {ticket[0][10]}",
        }
        
        for i, (label, value) in enumerate(details.items()):
            ttk.Label(frame, text=label, font=('calibri', 11, 'bold')).grid(row=i+1, column=0, sticky='w', padx=10, pady=2)
            ttk.Label(frame, text=value).grid(row=i+1, column=1, sticky='w', padx=10, pady=2)
    ...
    def print_boarding_pass(self):
        if not adms.hasbooked(self.mobno.get()):
            ...
            tk.Label(pass_frame, text=flight_name, font=("Helvetica", 18, "bold"), bg="white").grid(row=0, column=0, columnspan=4, pady=(0, 15))
            ...
            tk.Label(pass_frame, text=ticket_data[1], font=("Helvetica", 14, "bold"), bg="white").grid(row=2, column=0, columnspan=2, sticky="w", padx=5)
            
            tk.Label(pass_frame, text="FLIGHT", font=("Helvetica", 8, "italic"), bg="white").grid(row=1, column=2, sticky="w")
            tk.Label(pass_frame, text=ticket_data[2], font=("Helvetica", 14, "bold"), bg="white").grid(row=2, column=2, columnspan=2, sticky="w", padx=5)

            # Departure and Arrival
            tk.Label(pass_frame, text="FROM", font=("Helvetica", 8, "italic"), bg="white").grid(row=3, column=0, sticky="w", pady=(10, 0))
            tk.Label(pass_frame, text=ticket_data[3], font=("Helvetica", 16, "bold"), bg="white").grid(row=4, column=0, columnspan=2, sticky="w", padx=5)

            tk.Label(pass_frame, text="TO", font=("Helvetica", 8, "italic"), bg="white").grid(row=3, column=2, sticky="w", pady=(10, 0))
            tk.Label(pass_frame, text=ticket_data[4], font=("Helvetica", 16, "bold"), bg="white").grid(row=4, column=2, columnspan=2, sticky="w", padx=5)

            # Date, Class, and Seat
            tk.Label(pass_frame, text=f"DATE: {ticket_data[6]}", font=("Helvetica", 11), bg="white").grid(row=5, column=0, sticky="w", pady=(15, 0))
            tk.Label(pass_frame, text=f"CLASS: {ticket_data[5]}", font=("Helvetica", 11), bg="white").grid(row=5, column=1, columnspan=2, sticky="w", pady=(15, 0))
